...
| ID | Ngày chạy | Tên thử nghiệm | Trạng thái | Centralized (Acc/F1/mAP) | Local-Only Avg (Acc/F1/mAP) | Federated (Acc/F1/mAP) | Chi tiết |
| :---: | :---: | :--- | :---: | :---: | :---: | :---: | :--- |
...
> **Đây KHÔNG phải EXP journal**. Đây là **log tiến độ build** để resume sau khi context compact.
...
<a id="exp-011"></a>
### EXP-011 — PPE Detection GPU Simulation Baseline
- **Mã Thử Nghiệm:** EXP-011
- **Ngày Thực Hiện:** 2026-06-29
- **Trạng Thái:** Thành công
- **Git Commit Hash:** `3199c01`

---

#### 1. Mục Tiêu (Objective)
- Chạy baseline PPE object detection đầu tiên trên GPU RTX3060 với ảnh thật.
- Kiểm chứng pipeline detection hiện tại ở đủ 3 mode: `centralized`, `local-only`, `federated`.
- Đo mAP per-client và chi phí truyền thông của FedAvg khi chỉ aggregate Faster R-CNN detection head.
- Xác nhận detection track có thể chuyển từ smoke CPU nhỏ sang simulation GPU đủ lớn trước khi deployment thật.
...
- **Dữ liệu & phân chia client:**
  ...
  - Số client: 3 (`site-a`, `site-b`, `site-c`).
  - Validation: 60 ảnh/client, tổng 180 ảnh validation.
  - Split theo PPE class-presence skew:
    - `site-a`: helmet-heavy.
    - `site-b`: face-mask-medical-heavy.
    - `site-c`: glasses/gloves-heavy, thiếu một số lớp như helmet/vest/mask trong validation.
- **Class order cho `map_per_class`:**
  `helmet`, `safety-vest`, `safety-suit`, `face-mask-medical`, `gloves`, `glasses`, `ear-mufs`, `face-guard`.
- **Siêu tham số:**
  - Image size: `512`
  - Batch size: `2`
...
| Chế Độ Huấn Luyện (Mode) | Global mAP@0.5:0.95 | Global mAP@0.5 | Global mAP@0.75 | Tổng Thời Gian Train | Kích Thước Update (Per Round) | Tổng Chi Phí Truyền Thông (Comm. Cost) |
| :--- | :---: | :---: | :---: | :---: | :---: | :---: |
| **Centralized** (Pooled data, reference) | 0.1179 | 0.2755 | 0.0771 | 849.83s | 58,204,640 bytes | N/A |
| **Local-Only** (Không collab) | 0.0657 | 0.1601 | 0.0385 | 428.53s | 58,204,640 bytes | N/A |
| **Federated** (FedAvg head-only) | 0.0951 | 0.2351 | 0.0681 | 2,205.77s | 58,204,640 bytes | 1,746,139,200 bytes |

#### 4. Kết Quả Chi Tiết Theo Từng Client (Per-Client Metrics)
Mode `federated` sau 5 round.

| Client ID / Site | Số Lượng Validation | Local mAP@0.5:0.95 | Local mAP@0.5 | Local mAP@0.75 | Ghi Chú Đặc Điểm Dữ Liệu |
| :--- | :---: | :---: | :---: | :---: | :--- |
| **site-a** | 60 | 0.1085 | 0.2386 | 0.0913 | Helmet-heavy; histogram `helmet=300`, `gloves=169`, `safety-vest=98`, `glasses=73` |
| **site-b** | 60 | 0.1092 | 0.2705 | 0.0679 | Face-mask-heavy; histogram `face-mask-medical=249`, `gloves=126`, `glasses=74` |
| **site-c** | 60 | 0.0678 | 0.1963 | 0.0450 | Glasses/gloves-heavy; validation thiếu `helmet`, `safety-vest`, `face-mask-medical` |

Per-class AP chi tiết được lưu trong `outputs/EXP-011/federated_metrics.json` theo class order:
`helmet`, `safety-vest`, `safety-suit`, `face-mask-medical`, `gloves`, `glasses`, `ear-mufs`, `face-guard`.
Giá trị `-1.0` nghĩa là class không có ground-truth hợp lệ trong shard đánh giá tương ứng.

Per-class AP của mode `federated` sau 5 round:

| Class | site-a AP | site-b AP | site-c AP | Nhận xét ngắn |
| :--- | :---: | :---: | :---: | :--- |
| `helmet` | 0.3152 | N/A | N/A | Tốt nhất ở `site-a`, nhưng chưa đánh giá được ở `site-b/site-c` do thiếu hoặc không đủ ground-truth hợp lệ. |
| `safety-vest` | 0.2473 | 0.1688 | N/A | Học được tín hiệu tương đối rõ ở các site có object. |
| `safety-suit` | 0.0496 | 0.0305 | 0.0999 | AP còn thấp dù có mặt ở cả 3 site; có thể bị kéo bởi ít mẫu và biến thể hình dạng. |
| `face-mask-medical` | 0.0147 | 0.1707 | N/A | Phụ thuộc mạnh vào `site-b`; `site-a` có ít hơn và AP rất thấp. |
| `gloves` | 0.0478 | 0.0968 | 0.1073 | Có mặt nhiều nhưng AP chưa cao, có thể do vật thể nhỏ và dễ lẫn nền. |
| `glasses` | 0.0255 | 0.0791 | 0.0858 | AP thấp so với tần suất xuất hiện; khả năng do object nhỏ/khó nhìn ở image size 512. |
| `ear-mufs` | 0.0000 | N/A | 0.0459 | Một trong các lớp kéo mAP xuống rõ nhất, số mẫu ít và khó học ổn định. |
| `face-guard` | 0.1676 | N/A | 0.0000 | Có tín hiệu ở `site-a`, nhưng thất bại ở `site-c`; cần xem lại phân phối và chất lượng annotation. |

#### 5. Đường Hội Tụ Federated (Distributed Weighted)

| Round | mAP@0.5:0.95 | mAP@0.5 | mAP@0.75 |
| :---: | :---: | :---: | :---: |
| 1 | 0.0441 | 0.1040 | 0.0332 |
| 2 | 0.0798 | 0.2076 | 0.0539 |
| 3 | 0.0880 | 0.2069 | 0.0622 |
| 4 | 0.0937 | 0.2208 | 0.0732 |
| 5 | 0.0951 | 0.2351 | 0.0681 |

#### 6. Quan Sát & Phân Tích (Observations & Rationale)
- **EXP-011 hoàn thành mục tiêu Phase A quan trọng nhất:** detection pipeline chạy được trên GPU với dữ liệu thật, pretrained detector, mAP per-client, đủ 3 mode baseline và FedAvg head-only.
- **Centralized là trần tham chiếu tốt nhất:** mAP@0.5 `0.2755`, mAP@0.5:0.95 `0.1179`. Đây không phải điều kiện FL privacy thực tế, nhưng cho biết pooled reference vẫn tốt hơn các mode phân tán.
- **Federated vượt local-only rõ ràng:** FedAvg đạt mAP@0.5:0.95 `0.0951` so với local-only `0.0657`, và mAP@0.5 `0.2351` so với `0.1601`. Điều này cho thấy collaboration giữa site có ích trong detection, dù mới train head và chỉ 5 round.
- **Non-IID vẫn hiện rõ:** `site-c` thấp nhất trong federated (`mAP 0.0678`) do class distribution khác và thiếu nhiều lớp trong validation; `site-a/site-b` gần nhau quanh `0.108-0.109`.
- **Các lớp kéo mAP xuống chủ yếu:** `ear-mufs`, `safety-suit`, `glasses`, `gloves` và `face-guard` ở `site-c`. Đây là nhóm nên được kiểm tra annotation/scale object trước khi kết luận thuật toán FL yếu; nhiều lớp vừa ít mẫu vừa là vật thể nhỏ.
- **Class imbalance làm metric dao động theo site:** một số AP là N/A vì validation shard không có ground-truth hợp lệ cho lớp đó. Khi sang deployment thật, distributed evaluation phải ghi rõ class histogram từng client, nếu không global mAP sẽ dễ bị hiểu sai.
- **Hội tụ có tín hiệu tăng đều:** mAP@0.5:0.95 tăng từ `0.0441` round 1 lên `0.0951` round 5. Chưa thấy chững mạnh ở round 5, nhưng tăng thêm round sẽ đánh đổi communication rất lớn.
- **Communication là bottleneck thật:** detection head update size `58,204,640 bytes` mỗi round; 3 client × 5 round × gửi/nhận tạo `1,746,139,200 bytes` (~1.75 GB decimal). Đây là khác biệt rất lớn so với stage-1 classification head vài KB.
- **Runtime hợp lý nhưng nặng hơn classification:** centralized ~14.2 phút, local-only ~7.1 phút, federated ~36.8 phút. Peak RSS process chính khoảng `2.57 GB`; số này chưa phải GPU VRAM peak.

#### 7. Tài Liệu Hướng Dẫn Tái Lập (Reproducibility & Artifacts)
- **Lệnh chạy đã dùng:**
  ```bash
  python scripts/run_detection_sim.py \
    --mode all \
    --manifest configs/datasets/ppe_detection_exp011_manifest.csv \
    --root-dir data/ppe \
    ...
    --num-workers 0
  ```
- **Đường dẫn outputs:** `outputs/EXP-011/`
- **File output chính:**
  - `outputs/EXP-011/centralized_metrics.json`
  - `outputs/EXP-011/local_only_metrics.json`
  - `outputs/EXP-011/federated_metrics.json`
  - `outputs/EXP-011/summary.json`
- **Manifest tái lập:** `configs/datasets/ppe_detection_exp011_manifest.csv` đã có trong workspace, gồm 900 mẫu + header (300 mẫu/site, train/val theo manifest).
